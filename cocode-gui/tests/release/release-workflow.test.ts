...
test("exposes signed Linux DEB/RPM release workflow with native architecture jobs", () => {
	...
	assert.match(workflow, /rpm --import -/)
	...
	assert.equal(workflow.indexOf("\n  upload-release:"), workflow.lastIndexOf("\n  upload-release:"))
...
test("updates the os-linux draft release only after installed DEB and RPM smoke", () => {
	...
	assert.match(workflow, /cocode-linux-repository-/)
	assert.match(workflow, /Upload verified Linux release snapshot/)
	assert.match(workflow, /needs:\s*\[verify-repository\]/)
	assert.match(workflow, /finalize-linux-release-assets\.mjs/)
