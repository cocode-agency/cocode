# RFC：把 DSH 产品改动抽成 Cocode 插件（已实现）

- 状态：已实现并完成二次审查（2026-08-31）
- 日期：2026-08-28
- 范围：DSH npm 依赖、`packages/cocode/*` 产品插件、Electron overlay、Host Supervisor `--patch`、Renderer `file://` loopback
- 审查目标：确认产品能力已离开 DSH 快照、插件挂载正确、没有双轨实现、没有漏抽或误抽

当前仓库不保留 `packages/client` 或 `vendor` 的 DSH 源码快照；上游 DSH 通过 npm 依赖加载，Cocode 产品行为通过 `packages/cocode/*` 插件挂载。

本文件给后续模型做审查用。它记录已经落地的设计、实现位置、已知缝和未跑过的验证，不是一份待实现的方案。

## 1. 摘要

Cocode 产品改动禁止写进本地 DSH 源码快照。本轮把迁移前曾存在于快照里的产品行为抽到
`packages/cocode/*`，由 Cordis overlay 挂载；抽完后删除快照并改用 npm 上游包，不保留
deprecated 双轨。
...
1. npm 发布的 DSH 客户端是上游依赖。新产品进 `packages/cocode/*`。
2. 产品走 Cordis overlay，不把整份上游客户端重新打成本地 fork 或 pnpm patch。
...
6. 不要 disable `ui-theme`。AppearanceRow 用 shadow 藏。`installThemeStyles` 必须保留在 npm 上游包中。
...
11. Overlay / staging 只能做组合接线；产品 UI、文案和业务规则不得在其中复制第二份。
...
- 字号：`localStorage` key `cocode.message.fontSize`，写 Cocode 自有变量 `--cocode-message-font-size`，再由 appearance 插件的样式桥接到 DSH 共享内容字号轴 `--dsh-content-font-size`。conversation 正文与 Composer（包括 placeholder）读取该轴，插件负责赋值。
...
- 以 npm 上游 `ui-message-feedback` 为基线抽出产品版；`account.ts` 走 `window.desktopApi.account`。
...
- 以 npm 上游 `ui-settings-models` 为基线抽出带 account-gate 的产品版。
- overlay **disable** 上游 `ui-settings-models`。
- 测试已迁入插件：`tests/account-gate.client.spec.ts`、`tests/provider-tag.client.spec.ts`。

### 3.2 上游 npm 包恢复与迁移结果（历史记录）

以下记录的是迁移前曾存在于本地 DSH 快照的产品点；当前仓库已经删除该快照，运行时
直接使用 npm 上游包，产品实现位于 `packages/cocode/*`：
...
| Settings 面板暗色 `#101012` | npm 上游保持默认；产品 CSS 在 `cocode-desktop` |
| Sidebar titlebar DOM/CSS | 已删 |
| Conversation `data-settings-panel` 拖拽覆盖 | 已迁到 `cocode-desktop` chrome CSS |

产品文件禁止清单（当前仓库不存在 DSH 快照，测试会断言路径和产品实现均不回归）：
...
`packages/cocode/ARCHITECTURE.md` 已改成「产品在 `packages/cocode`」，并写明
workbench dock 槽位仍由 npm DSH layout 插件声明。
历史遗留、没有 manifest 或构建入口的 `packages/cocode/cocode-sidebar` 样式文件已删除；
sidebar 产品能力只保留在可构建的 `cocode-workbench` / `cocode-desktop` 插件中。
...
1. **workbench dock 槽位仍由 npm DSH `ui-layout` 插件声明**。如果上游未来移除该扩展点，诚实抽法是 overlay-replace 整个 `ui-layout`；本轮不引入第二份本地 DSH 源码。
2. **没有 Electron 端到端验证**。未在真实 Desktop 窗口里点：侧栏拖拽、外观字号、Nut 锁定、反馈、快捷方式图标、`file://` 设置面、恢复条。
3. **没有跑完整上游 DSH vitest**。当前仓库不再包含 DSH 源码快照；本轮验证的是 npm 包加载、Cocode 插件构建、Host patch、Renderer bundle 和 GUI 公共测试。
4. **`createDshDesktopPatch` 未被 spawn 调用**。运行时只认 Supervisor patch。GUI helper 仅作为 Electron overlay 合同测试面，不能成为第二份产品实现；产品逻辑仍必须放在 `packages/cocode/*`。
...
当前仓库不应存在 `packages/client` 或 `vendor` 目录；产品实现只允许位于
`packages/cocode/*` 或明确的 Host/Renderer overlay 接线处：
...
- `--cocode-message-font-size`（appearance 的本地字号值）
- `--dsh-content-font-size`（conversation 与 Composer CSS 读取，由 appearance 的样式桥接）
...
未跑：Electron 手工/浏览器 E2E、完整上游 DSH vitest、`pnpm run typecheck` 全仓。
...
# 不得重新引入 DSH 源码快照或 vendor
test ! -e packages/client
test ! -e vendor

# 产品实现应位于 Cocode 插件或明确 overlay 接线处
rg -n "AppearanceSection|account-gate|agencyMessageFeedbackRemote|RuntimeRecoveryBanner|cocode:open-settings|data-desktop-titlebar-drag" packages/cocode
...
发现问题时优先改 `packages/cocode/*` 或 overlay，不要把产品逻辑写回上游 DSH npm 包。
