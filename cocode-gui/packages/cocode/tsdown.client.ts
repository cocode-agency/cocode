...
function optionalStringArray(
  ...
  if (!Array.isArray(value) || value.some(item => typeof item !== 'string')) {
    throw new Error(`client bundle: ${subject} ${field} must be a string array`)
  }
 ...
 *    which collapses the npm/index cache split.
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      // Resolve ordinary Cocode plugin dependencies from the importing
      // package. DSH framework packages are installed from npm and stay on the
      // explicit module-table or inline-safe paths below.
      name: 'cocode-client-dependency-resolver',
      ...
      resolveId(source: string) {
        ...
        if (VENDORED_LIBRARY.test(source)) return null // shared library: inline, no duplicate identity
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        const abs = cssAssetPath(source, importer)
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        const abs = cssAssetPath(stylesheet, importer)
