...
export function MessageFeedbackActions({ messageId, ensure, rate, toggle, clearNote, useFeedback, t }: MessageFeedbackActionProps) {
  ...
  useEffect(() => {
    // Account reset deliberately returns the shared controller to `cold`.
    // Controls stay mounted across that transition, so retire the per-control
    // one-shot gate and let the next interaction seed the new account.
    if (feedbackStatus === 'cold') seeded.current = false
  }, [feedbackStatus])
