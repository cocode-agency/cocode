...
export class MessageFeedbackController implements HostObservable<MessageFeedbackView> {
  ...
  /** Monotonic account/session lifecycle token. Async work may publish only while it matches. */
  ...
  rate(
    ...
    return this.mutate(async (generation) => {
      const observed = this.view.items.get(messageId)
      return await this.putCommitted(messageId, rating, note ?? observed?.note, observed, generation)
    })
  }
  ...
  toggle(messageId: MessageId, rating: MessageFeedbackRating): Promise<MessageFeedbackActionResult> {
    return this.mutate(async (generation) => {
      const observed = this.view.items.get(messageId)
      if (observed?.rating === rating) return await this.deleteCommitted(messageId, observed, generation)
      return await this.putCommitted(messageId, rating, observed?.note, observed, generation)
    })
  }
  ...
  clearNote(messageId: MessageId): Promise<MessageFeedbackActionResult> {
    return this.mutate(async (generation) => {
      ...
      return await this.putCommitted(messageId, observed.rating, undefined, observed, generation)
    })
  }
  ...
  clear(messageId: MessageId): Promise<MessageFeedbackActionResult> {
    return this.mutate(async (generation) => {
      ...
      return await this.deleteCommitted(messageId, observed, generation)
    })
  }
  ...
  private async putCommitted(
    ...
    if (result.ok) {
      this.commit(messageId, result.value, generation)
      return OK
    }
    if (result.error.code === 'version-conflict') this.commit(messageId, result.error.current, generation)
  ...
  private async deleteCommitted(
    ...
    const result = carried.value
    if (result.ok) {
      this.commit(messageId, null, generation)
      return OK
    }
    if (result.error.code === 'version-conflict') this.commit(messageId, result.error.current, generation)
    return fail(result.error.code)
  ...
  /** Drop cached items so the next ensure() reads the current account state. */
  reset(): void {
    if (this.disposed) return
    this.generation += 1
    this.loadPromise = null
    // Let the new account start immediately; old queued operations retain their
    // generation and become no-ops when they reach the wire or return.
    this.operationTail = Promise.resolve()
    this.publish(INITIAL_VIEW, this.generation)
  }
  ...
  private async load(generation: number): Promise<MessageFeedbackActionResult> {
    try {
      const carried = await this.remote.list({ sessionId: this.sessionId })
      if (!this.isCurrent(generation)) return OK
      if (!carried.ok) {
        this.publish({ status: 'error', items: this.view.items, error: carried.error.message }, generation)
        return carrierFailure(carried.error)
      }
      const result = carried.value
      if (!result.ok) {
        this.publish({ status: 'error', items: this.view.items, error: describe(result.error.code) }, generation)
        return fail(result.error.code)
      }
      const items = new Map<MessageId, MessageFeedbackItem>()
      for (const item of result.value.items) items.set(item.messageId, item)
      this.publish({ status: 'ready', items, error: null }, generation)
      return OK
    } catch (error) {
      if (!this.isCurrent(generation)) return OK
      const message = error instanceof Error ? error.message : 'message feedback list failed'
      this.publish({ status: 'error', items: this.view.items, error: message }, generation)
   ...
   */
  private mutate(
    operation: (generation: number) => Promise<MessageFeedbackActionResult>,
    ...
    const guarded = async (): Promise<MessageFeedbackActionResult> => {
      ...
      if (options.seed !== false) {
        const loaded = await this.ensureFor(generation)
   ...
   */
  private commit(messageId: MessageId, item: MessageFeedbackItem | null, generation: number): void {
  ...
  private publish(view: MessageFeedbackView, generation = this.generation): void {
    if (!this.isCurrent(generation)) return
  ...
  private ensureFor(generation: number): Promise<MessageFeedbackActionResult> {
    if (!this.isCurrent(generation)) return Promise.resolve(OK)
    if (this.view.status === 'ready') return Promise.resolve(OK)
    return this.refresh()
  }
