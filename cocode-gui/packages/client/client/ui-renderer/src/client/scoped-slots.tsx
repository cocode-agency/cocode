...
function bindInjectHooks(face: InjectedProps): InjectedProps {
  ...
  for (const [name, source] of Object.entries(sources as Record<string, HostObservable<unknown>>)) {
    ...
    bound[hookName] = observableHook(source, `inject.hooks.${name}`)
  }
  return bound
}
...
function cachedSlotInject(face: object | undefined): BoundSlotInject {
  ...
  for (const [name, definition] of Object.entries(definitions as Record<string, unknown>)) {
    ...
    if (typeof definition === 'function') {
      ...
      props[hookName] = observableHook(definition as HostObservable<unknown>, `slot.inject.hooks.${name}`)
    }
  }
...
function standardProps(
  ...
  if (cache === undefined) {
    cache = {
      root: {
        // Older/HMR-overlapped runtime faces can briefly expose the standard
        // kit before one of its observable sources is installed. Binding an
        // optional source keeps slot assembly alive; consumers that need the
        // feed still receive the normal hook once the current host provides it.
        useSessions: maybeObservableHook(host.sessions?.list, 'sessions.list'),
        useWorkspaces: maybeObservableHook(host.workspaces?.list, 'workspaces.list'),
      },
      session: new WeakMap(),
  ...
  for (const [name, source] of Object.entries(info.hooks)) {
    ...
    if (scope === 'session-maybe') {
      standard[hookName] = maybeObservableHook(source, `session-maybe.hooks.${name}`)
    } else {
      if (source === undefined) throw new SlotAssemblyError(`strict session hook '${name}' has no source`)
      standard[hookName] = observableHook(source, `session.hooks.${name}`)
    }
  }
...
function standardKit(
  ...
  // A session-maybe slot may still declare a root-scoped store (the
  // conversation shell does this for its logo preference). Only suppress
  // resolution when the slot's own store is session-scoped and there is no
  // current session; resolving a root store with an undefined scope key is
  // valid and must still provide useStore in the blank shell.
  const slotSpec = host.specOf(slotKey)
  const store = slotSpec?.scope === 'session' && info?.sessionId === undefined
    ? undefined
...
function SessionEntry({ entry, ownerProps, info, slotKey, slotInjected, hookContext, hasHookContext }: {
  ...
  const { kit, standard, actions } = standardKit(host, entry, 'session', info, slotKey)
...
function SessionMaybeEntryBody({ entry, ownerProps, info, slotKey, slotInjected, hookContext, hasHookContext }: {
  ...
  const host = useHost()
  const Comp = entry.component as FC<InjectedProps>
  const { kit, standard, actions } = standardKit(host, entry, 'session-maybe', info, slotKey)
...
function RootEntry({ entry, ownerProps, slotKey, slotInjected, hookContext, hasHookContext }: {
  ...
  const host = useHost()
  const Comp = entry.component as FC<InjectedProps>
  const { kit, standard, actions } = standardKit(host, entry, 'root', undefined, slotKey)
