...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['slots', 'conversation', 'sessions', 'workspaces'], (scope: ClientContext) => {
    ...
    // Resolve service faces while this injected scope is active. The scope
    // proxy is lifecycle-bound; callbacks retained by the seat may run after
    // a scope transition and must not dereference it after deactivation.
    ...
    scope.effect(() => {
      ...
      const settingsMoved = remote.$on('settings/document-updated', (ns) => {
        if (ns !== AGENT_PRESET_SETTINGS_NS) return
        void seat.load()
      })
      ...
      const presetSelected = remote.$on('agent-preset/selected', (sessionId, agentPreset) => {
      ...
      const chip = slots.register({
        name: 'conversation.hero.agentPreset',
        locale: 'settings.agentPreset',
        inject: seatInjected,
      }, AgentPresetSeat)
      const label = slots.register({
