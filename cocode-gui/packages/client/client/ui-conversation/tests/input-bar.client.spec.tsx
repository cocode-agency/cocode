...
describe('Enter semantics', () => {
  ...
  it('keeps the owning placeholder or ordinary guidance when whole-queue steering is unavailable', () => {
    expect(bench({ running: true }).textarea.placeholder).toBe('规划与构建，@ 引用文件和上下文，/ 执行技能与命令  ')
    expect(bench({ queue: [row('q-1')] }).textarea.placeholder).toBe('规划与构建，@ 引用文件和上下文，/ 执行技能与命令  ')
    expect(bench({ running: true, queue: [row('q-1')], draft: '消息' }).textarea.placeholder).toBe('规划与构建，@ 引用文件和上下文，/ 执行技能与命令  ')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      subagent: {
    ...
    }).textarea.placeholder).toBe('规划与构建，@ 引用文件和上下文，/ 执行技能与命令  ')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      placeholder: '上层指定提示',
...
describe('running and lock semantics', () => {
  ...
  it('disabled state shows the unavailable placeholder; custom placeholder wins', () => {
    ...
    expect(textarea.placeholder).toBe('会话不可用')
    const live = bench()
    expect(live.textarea.placeholder).toBe('规划与构建，@ 引用文件和上下文，/ 执行技能与命令  ')
  ...
  it('the plan projection swaps the placeholder while its effective target is plan mode', () => {
    ...
    expect(leaving.textarea.placeholder).toBe('规划与构建，@ 引用文件和上下文，/ 执行技能与命令  ')
...
describe('command launcher chrome and control seats', () => {
  ...
  it('the Access chip renders the projection value and submits a non-Full-access pick directly', async () => {
    ...
    // Localized labels are presentation only; the menu ids stay machine names.
    ...
    expect(items.map(o => o.textContent)).toEqual(['只读', '工作区可写', '完全权限'])
