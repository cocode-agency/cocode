...
interface StoreAxisRecord {
  /** Effective store scope: optional shells use one root instance in both session states. */
  scope: SlotScope
...
export class SlotRegistry extends Service {
  ...
  private _acquire(handle: EngineStoreHandle, scope: SlotScope): void {
    // A session-maybe entry remains mounted before a session exists. Its store
    // therefore needs a root identity; otherwise the renderer cannot resolve
    // a shell store during the blank state. Strict session entries retain the
    // per-session axis.
    const effectiveScope: SlotScope = scope === 'session' ? 'session' : 'root'
    const record = this._stores.get(handle)
    if (record === undefined) {
      this._stores.set(handle, { scope: effectiveScope, refs: 1, instances: new Map() })
      return
    }
    record.refs += 1
  }
