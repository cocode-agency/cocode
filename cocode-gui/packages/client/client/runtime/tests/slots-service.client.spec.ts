...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    't.optional': { kind: 'single'; scope: 'session-maybe' }
  }
}
...
describe('store instance axis', () => {
  ...
  it('resolves a session-maybe shell store without a current session', async () => {
    const bench = await boot()
    const host = captureHost(bench, { 't.optional': { kind: 'single', scope: 'session-maybe' } })
    const { handle } = fakeHandle()
    bench.erased.register({ name: 't.optional', store: handle }, C)
    const [entry] = host.entriesOf('t.optional')

    const instance = host.storeOf(entry as never, undefined)

    expect(instance).toBeDefined()
    expect(handle.create).toHaveBeenCalledWith()
  })
