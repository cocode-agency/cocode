...
describe('ModelsSection', () => {
  ...
  it('does not expose edit or delete actions for the account-managed Cocode Nut provider', async () => {
    const scripted = scriptedFace()
    const namespaces = wireNamespaces()
    const piAi = namespaces.find(namespace => namespace.ns === 'llm-pi-ai')!
    const route = { apiKeyEnv: 'COCODE_NUT_API_KEY', baseURL: 'https://cocode.example' }
    piAi.value = { ...piAi.value, providers: { ...(piAi.value.providers as Record<string, unknown>), 'cocode-nut': route } }
    piAi.user = { ...piAi.user, providers: { ...(piAi.user?.providers as Record<string, unknown>), 'cocode-nut': route } }
    ...
    scripted.face.llm.providers.mockImplementation(() => Promise.resolve(ok({
      providers: [
        { provider: 'cocode-nut', displayName: 'Cocode Nut', settingsNs: 'llm-pi-ai', settingsPath: ['providers', 'cocode-nut'], active: true },
      ],
    })))
    scripted.face.credentials.describe.mockImplementation(() => Promise.resolve(ok({
      credentials: { COCODE_NUT_API_KEY: { configured: true, writable: true } },
    })))
    ...
    expect(screen.getByText('Cocode Nut')).toBeTruthy()
    expect(screen.queryByRole('button', { name: providerCopy(en.editProvider, { provider: 'cocode-nut', displayName: 'Cocode Nut' }) })).toBeNull()
    expect(screen.queryByRole('button', { name: providerCopy(en.removeProvider, { provider: 'cocode-nut', displayName: 'Cocode Nut' }) })).toBeNull()
  })
