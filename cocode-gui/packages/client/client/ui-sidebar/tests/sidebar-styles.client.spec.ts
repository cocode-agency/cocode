...
describe('SidebarRoot.module.css', () => {
  it('shares and cancels the wide shell trailing padding structurally', () => {
    ...
    expect(root?.get('padding')).toBe('0 var(--dsh-sidebar-inline-padding) 6px var(--dsh-sidebar-inline-padding)')
  ...
  it('moves the four upper controls while the settings seat only fades', () => {
    ...
    expect(declarations('.railIn .footArea')?.get('animation')).toBe('none')
    expect(declarations('.railIn .footArea')?.get('opacity')).toBe('1')
    expect(css).toMatch(
  ...
  it('keeps the Cocode brand artwork and Electron drag contract', () => {
