...
describe('computeColumns', () => {
  it('step 1: everything fits at preferred widths', () => {
    ...
    expect(cols).toEqual({ sidebar: 280, center: 1920 - 280 - 360, workbench: 0, details: 360 })
  })
  ...
  it('step 2: details shrinks first, center pinned at min', () => {
    ...
    expect(cols).toEqual({ sidebar: 280, center: CENTER_MIN, workbench: 0, details: 330 })
  })

  it('boundary: exactly at the step-1/step-2 seam', () => {
    const cols = computeColumns(300 + 360 + CENTER_MIN, open(300), open(360))
    expect(cols).toEqual({ sidebar: 300, center: CENTER_MIN, workbench: 0, details: 360 })
    const one = computeColumns(300 + 360 + CENTER_MIN - 1, open(300), open(360))
    expect(one).toEqual({ sidebar: 300, center: CENTER_MIN, workbench: 0, details: 359 })
  })
  ...
  it('step 3: details auto-closes when its min still starves center — sidebar holds its preference', () => {
    ...
    expect(cols).toEqual({ sidebar: 280, center: 930, workbench: 0, details: 0 })
  })
  ...
  it('sidebar-closed narrow window: details concedes then auto-closes', () => {
    ...
    expect(fits).toEqual({ sidebar: SIDEBAR_COLLAPSED, center: CENTER_MIN, workbench: 0, details: DETAILS_MIN })
...
describe('computeColumns — degenerate viewports', () => {
  ...
  it('keeps an opened right workbench before conceding center width', () => {
    expect(computeColumns(1280, open(SIDEBAR_DEFAULT), closed(DETAILS_DEFAULT), 360))
      .toEqual({ sidebar: SIDEBAR_DEFAULT, center: 640, workbench: 360, details: 0 })
    expect(computeColumns(1000, open(SIDEBAR_DEFAULT), closed(DETAILS_DEFAULT), 360))
      .toEqual({ sidebar: SIDEBAR_DEFAULT, center: 360, workbench: 360, details: 0 })
  })
})
