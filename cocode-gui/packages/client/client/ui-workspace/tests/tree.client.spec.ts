...
describe('deriveGroups', () => {
  ...
  it('projects only read, idle sessions into the group archive action', () => {
    const archivable = summary('archivable', 3)
    const running = { ...summary('running', 2), running: true }
    const completed = { ...summary('completed', 1), completed: true }
    const groups = deriveGroups(
      list(archivable, running, completed),
      [workspace('first', ['archivable', 'running', 'completed'])],
      noArchive,
      view(['first']),
    )

    expect(groups[0]!.archivableSessionIds).toEqual([archivable.id])
  })
